...
- A set of **fact relations**  
    (F_1, F_2, …)  
    e.g. (F_{\text{aqpr}}) for `assessment_question_point_responses`.
    
- A set of **dimension relations**  
    (D_1, D_2, …)  
    (domain, region, calendar, etc.)
...
- A mapping from **logical attribute names** (e.g. `domain`, `assessment_date`) to:
...
> A named scalar function that, at runtime, is evaluated over a _group_ of fact rows (plus a filter context), and returns a number.
...
1. **Rowset transforms** (table-level, pre-aggregation):  
    ...
    e.g. applying time windows, shifting to last year, restricting to final assessments, etc.
    
2. **Metric transforms** (expression-level or higher-order):  
    They wrap one or more base metrics, but in practice just arrange to call those metrics on a _different_ rowset (e.g. last year’s rows).
...
- Attributes (columns) as (A, B, \dots).
...
- Example:  
    (F_{\text{aqpr}}(\text{domain_id}, \text{assessment_date}, \text{question_type}, \text{assessable_points}, \text{assessed_points}, \dots))
    

Each **dimension** is a relation:

- Example:  
    (D_{\text{domain}}(\text{domain_id}, \text{domain_name}, \dots))
    

A **logical attribute name** `domain` is mapped (by the semantic model) to some attribute in a relation, e.g.:

- `domain` ↦ (D_{\text{domain}}.\text{domain_name})
    
- And we know how to reach that dimension from a given fact:  
    e.g. (F_{\text{aqpr}}.\text{domain_id} = D_{\text{domain}}.\text{domain_id})
...
1. **Row-level expressions**:  
    ...
    - Semantics:  
        (\llbracket e \rrbracket(t)) – standard expression evaluation.
        
2. **Aggregation operators**:  
    Given an attribute-level expression (e) and a group of rows (R'_g), define:
    
    - (\textsf{SUM}(e, R'_g) := \sum_{t \in R'_g} \llbracket e \rrbracket(t))
...
```dsl
metric total_points_last_week =
  sum(points where assessment_date in LAST_WEEK)
```

Semantics:

1. Let (\psi_{\text{last_week}}(t)) be the predicate “`assessment_date` is in the last week relative to the evaluation context” – this is a boolean function over rows.
    
2. Within group (R'_g), define a restricted relation:  
    [  
    R'_{g,\text{last_week}} := \sigma_{\psi_{\text{last_week}}}(R'_g)  
    ]
    
3. Then:  
    [  
    \llbracket \text{total_points_last_week} \rrbracket(R'_g)  
    :=  
    \textsf{SUM}(\text{points}, R'_{g,\text{last_week}})  
    ]
...
[  
F(\text{tradyrwkcode}, \text{factcolumn}, \dots)  
]
...
2. Join fact with this transform row on the **lastyear form**:  
    [  
    F^{\text{LY}}_w := F \bowtie_{F.\text{tradyrwkcode} = T.\text{tradyrwkcode_lastyear}} T_w  
    ]
    
3. Then the metric at week (w) is:  
    [  
    \text{SalesLastYear}(w) := \sum_{t \in F^{\text{LY}}_w} t.\text{factcolumn}  
    ]
...
We want a relation parameterized by “current code” (c), but fact rows grouped by the **lastyear code**. At a higher level, there is an RA expression of the form:

- join: (F \bowtie_{F.\text{tradyrwkcode} = T.\text{tradyrwkcode_lastyear}} T),
    
- group by `T.tradyrwkcode` (current code),
    
- aggregate `F.factcolumn`.
    

That’s exactly what your SQL example does.
...
3. The transformed metric “Sum Sales Last Year” is:  
    [  
    \llbracket m_{\text{LY}} \rrbracket(R'_g) := \textsf{SUM}(\text{factcolumn}, R'^{\text{LY}}_g)  
    ]
...
This aligns perfectly with MicroStrategy’s behavior, but expressed through RA and rowset functions rather than attribute forms.
...
2. Let:  
    ...
    where (R'^{\text{LY}}_g) is obtained by a **rowset transform** (T_{\text{LY}}).
    
3. Then:  
    [  
    \llbracket \text{yoy_sales} \rrbracket(R'_g) :=  
    \frac{v_{\text{curr}} - v_{\text{ly}}}{v_{\text{ly}}}  
    ]
...
2. **Query layer**  
    ...
    - A grain (G = (d_1, \dots, d_k)): **dimensions**.
    ...
    - Build (R) by joining the base fact with required dimensions.
...
4. **Transforms layer**
    ...
    - **Table/rowset transforms**: RA expressions like joins with transform tables, window restrictions, time shifts.
        
    - **Metric transforms**: higher-order metric evaluators that change the rowset argument (e.g. last year) and then call the base metric evaluator.
        
5. **Invariant**
    
    - **Metrics are grain-agnostic**: they are defined only in terms of “given a group of fact rows and filters, compute a scalar”.
...
- pick one concrete metric from your domain (e.g. `percent_assessed` or a last-year variant),
...
Let’s nail one fully-worked “last year” metric end-to-end: DSL → AST → relational algebra → how it behaves like the MicroStrategy transform.

I’ll base it on your week example (trade year-week).
...
Semantics: for whatever grain the query chooses and whatever rows land in the group, just sum `factcolumn`.

### 1.2. Last-year variant (using a transform)

We expose a metric-level transform in the DSL:
...
- `sum_sales` is the base metric.
    
- `last_year(m, tradyrwkcode)` means:
    
    > “For each current week code, find the corresponding last-year week(s), then evaluate `m` on the rows whose `tradyrwkcode` is those last-year codes, but report the result at the _current_ week grain.”
...
- Knows there is a fact `fact_sales(tradyrwkcode, factcolumn, …)`.
...
const sumSalesMetric: MetricDefNode = {
  ...
  expression: {
    ...
    args: [
      { kind: "Identifier", name: "factcolumn" }
    ]
  }
};
```

After resolution, `factcolumn` becomes an `AttributeRef`:
...
const sumSalesResolvedExpr: ResolvedExprNode = {
  ...
  callee: "sum",
  args: [
    { kind: "AttributeRef", logicalName: "factcolumn" }
  ]
};
```
...
[  
R'_w := \sigma_{\text{tradyrwkcode} = w}(R')  
]
...
:= \textsf{SUM}(\text{factcolumn}, R'_w)  
= \sum_{t \in R'_w} t.\text{factcolumn}  
]
...
[  
R^{\text{LY}}_w := F \bowtie_{F.\text{tradyrwkcode} = T.\text{tradyrwkcode_lastyear}} T_w  
]

That’s exactly what your SQL pattern shows:

```sql
SELECT tf.tradyrwkcode,            -- current week
       SUM(ft.factcolumn)
FROM   fact_sales ft
JOIN   tradyrwk_transform tf
  ON   ft.tradyrwkcode = tf.tradyrwkcode_lastyear
WHERE  tf.tradyrwkcode = 202512
GROUP BY tf.tradyrwkcode;
```

Same logic, just in RA notation.

#### Last-year metric `sum_sales_last_year`

Now the metric semantics are:

[  
\llbracket \text{sum_sales_last_year} \rrbracket(R'_w)  
:= \textsf{SUM}(\text{factcolumn}, R^{\text{LY}}_w)  
= \sum_{t \in R^{\text{LY}}_w} t.\text{factcolumn}  
]
...
- For each current week (w),
    ...
    - sum `factcolumn` over those last-year rows,
        
    - but **attach the result back to the current week (w)** (the query grain).
        

That’s the precise RA version of “metric transform via rowset transform”.
...
[  
F \bowtie_{F.\text{tradyrwkcode} = T.\text{tradyrwkcode_lastyear}} \sigma_{T.\text{tradyrwkcode} = w}(T)  
]
...
That’s the full “last-year” variant:

- **DSL** definition,
    
- **AST** structure (base metric + metric transform),
    
- and formal **relational algebra semantics** that match both your engine design and MicroStrategy’s transform behavior.
